...
export const fallbackMovies = rawTmdbMovies.map((m, index) => {
  const hexId = index.toString(16).padStart(24, '0');
  const trailer = trailerPool[index % trailerPool.length];
  
  // Use a pseudo-random but deterministic genre assignment based on index to ensure consistency across refreshes
  const numGenres = index % 2 === 0 ? 2 : 1;
  const assignedGenres = [targetGenres[index % targetGenres.length]];
  if(numGenres === 2) {
      const secondGenre = targetGenres[(index + 1) % targetGenres.length];
      if(secondGenre !== assignedGenres[0]) assignedGenres.push(secondGenre);
  }
  ...
  return {
    ...
    releaseDate: new Date(2010 + (index % 12), 0, 1),
    ...
    rating: parseFloat(((index % 3) + 7.1).toFixed(1)),
    isTrending: index < 15,
    cast: [
      { name: "Star Actor", role: "Protagonist" },
      { name: "Supporting Actor", role: "Sidekick" }
    ]
  };
});
